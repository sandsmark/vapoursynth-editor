...
{
public:
    void setPrompt(const QChar &prompt)
    {
        m_prompt = prompt;
    }
    ...
    QStringRef userContents() const
    {
        return m_buffer.leftRef(m_userPos);
    }
    const QChar &prompt() const
    {
        return m_prompt;
    }
    ...
    void moveEnd()
    {
        m_userPos = m_pos = m_buffer.size();
    }

    void setHistoryAutoSave(bool autoSave)
    {
        m_historyAutoSave = autoSave;
    }
    void historyDown()
    {
    ...
    const QStringList &historyItems() const
    {
...
{
public:
    const Inputs &value() const
    {
        return m_value;
    }
    void setValue(const Inputs &value)
    {
        m_value = value;
    }
private:
...
public:
    ...
    {
        return isValid() && !last()->empty();
    }

    // Return true if this mapping can be used.
    bool isComplete() const
    {
        return m_lastValid != -1;
    }
    ...
    const Inputs &currentInputs() const
    {
...
public:
    ...
    int mvCount() const
    {
        return qMax(1, g.mvcount);
    }
    int opCount() const
    {
        return qMax(1, g.opcount);
    }
    int count() const
    {
        return mvCount() * opCount();
    }
    ...
    int rightDist() const
    {
        return block().length() - leftDist() - (isVisualCharMode() ? 0 : 1);
    }
    ...
    {
        return atBlockEnd() && block().length() > 1;
    }
    bool atDocumentEnd() const
    {
        return position() >= lastPositionInDocument(true);
    }
    bool atDocumentStart() const
    {
    ...
    bool moveToPreviousParagraph(int count = 1)
    {
        return moveToNextParagraph(-count);
    }
    ...
    bool canModifyBufferData() const
    {
        return m_buffer->currentHandler.data() == this;
    }
    ...
    bool isCommandLineMode() const
    {
        return g.mode == ExMode || g.subsubmode == SearchSubSubMode;
    }
    bool isInsertMode() const
    {
        return g.mode == InsertMode || g.mode == ReplaceMode;
    }
    ...
    {
        return g.visualMode != NoVisualMode;
    }
    bool isNoVisualMode() const
    {
        return g.visualMode == NoVisualMode;
    }
    bool isVisualCharMode() const
    {
        return g.visualMode == VisualCharMode;
    }
    bool isVisualLineMode() const
    {
        return g.visualMode == VisualLineMode;
    }
    bool isVisualBlockMode() const
    {
        return g.visualMode == VisualBlockMode;
    }
...
public:
    ...
    {
        return Range(position(), anchor(), g.rangemode);
    }
    ...
    int revision() const
    {
        return document()->availableUndoSteps();
    }
    ...
    void setDotCommand(const QString &cmd)
    {
        g.dotCommand = cmd;
    }
    void setDotCommand(const QString &cmd, int n)
    {
        g.dotCommand = cmd.arg(n);
    }
    ...
    CursorPosition markLessPosition() const
    {
        return mark('<').position(document());
    }
    CursorPosition markGreaterPosition() const
    {
        return mark('>').position(document());
    }
...
{
    transformText(range, [](const QString &) {
        return QString();
    });
}
...
{
    transformText(range, [](const QString & text) {
        return text.toLower();
    });
}
...
{
    transformText(range, [&str](const QString &) {
        return str;
    });
}
